          ...
          <div className="mt-12 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
            {ENT_PROFILE_SUBJECTS.map((subj, i) => (
              <Reveal key={subj.id} delay={i * 40}>
                ...
                >
                  <span
                    className={`flex h-11 w-11 items-center justify-center rounded-xl text-xl transition-transform duration-300 group-hover:scale-110 ${SUBJECT_COLORS[i % SUBJECT_COLORS.length]}`}
                  >
                    {subj.icon}
                  </span>
